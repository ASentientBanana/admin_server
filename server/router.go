...
func InitRoutes(engine *gin.Engine, db *gorm.DB) {
	...
	engine.PUT("/projects/positions", middleware.Authenticate, NewHandler(db, controllers.UpdateProjectPositions))
	// engine.PUT("/projects", middleware.Authenticate, NewHandler(db, controllers.UpdateProjects))
	engine.PUT("/projects/:id", middleware.Authenticate, NewHandler(db, controllers.UpdateProject))
	engine.POST("/projects", middleware.Authenticate, NewHandler(db, controllers.CreateProjects))
	engine.DELETE("/projects/:id", middleware.Authenticate, NewHandler(db, controllers.DeleteProjects))
